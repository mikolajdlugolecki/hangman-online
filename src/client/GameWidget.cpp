...
void GameWidget::init(const QString wordLength, const QString maxErrors, const QString maxSeconds)
{
    ...
    QString errorsLabel = QString::fromStdString(std::to_string(GameState::instance().currentErrors)) + " / " + maxErrors;
...
void GameWidget::guessPositions(std::vector<std::string> positions)
{
    ...
    {
        GameState::instance().word[std::stoi(pos)] = std::toupper(GameState::instance().lastGuessedLetter.toStdString()[0]);
    }
    ui->maskedWordLabel->setText(transformWord(GameState::instance().word));
}
...
{
    QString errorsLabel = QString::fromStdString(std::to_string(++GameState::instance().currentErrors)) + " / " + GameState::instance().maxErrors;
    ui->errorsLabel->setText(errorsLabel);
}
